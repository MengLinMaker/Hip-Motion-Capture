/*!
 * @file TinyBMX160.cpp
 * @brief define TinyBMX160 class infrastructure, the implementation of basic methods
 ...
 * @version  V1.0
 * @date  2021-10-20
 * @url https://github.com/DFRobot/TinyBMX160
 */
#include "TinyBMX160.h"

TinyBMX160::TinyBMX160(TwoWire *pWire)
{
  _pWire = pWire;
...
bool TinyBMX160::begin(uint8_t addr)
{
    _addr = addr;
    _pWire->begin();
    if (scan() == true){
...
int8_t TinyBMX160:: _softReset(sBmx160Dev_t *dev)
{
  int8_t rslt=BMX160_OK;
  ...
  if (rslt == BMX160_OK){
    TinyBMX160::defaultParamSettg(dev);
  }  
  return rslt;
}

void TinyBMX160::defaultParamSettg(sBmx160Dev_t *dev)
{
...
void TinyBMX160::setGyroRange(eGyroRange_t bits){
    switch (bits){
...
void TinyBMX160::setAccelRange(eAccelRange_t bits){
    switch (bits){
        case eAccelRange_2G:
...
void TinyBMX160::getAllData(sBmx160SensorData_t *magn, sBmx160SensorData_t *gyro, sBmx160SensorData_t *accel){
...
void TinyBMX160::writeBmxReg(uint8_t reg, uint8_t value)
{
    uint8_t buffer[1] = {value};
    writeReg(reg, buffer, 1);
}

void TinyBMX160::writeReg(uint8_t reg, uint8_t *pBuf, uint16_t len)
{
...
void TinyBMX160::readReg(uint8_t reg, uint8_t *pBuf, uint16_t len)
{
   _pWire->beginTransmission(_addr);
   _pWire->write(reg);
